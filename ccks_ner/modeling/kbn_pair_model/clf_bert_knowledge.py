...
def get_layer(inp_type, base_model):
    x_a = base_model.output
...
def build_model(lr=0.0, lr_d=0.0):
    ...
    embedding = BERTEmbedding(emn_path, 512)
    base_model = embedding.model

    out = get_layer(inp_type, base_model)
    ...
    """fine-tune"""
    model = Model(inputs=[base_model.inputs, base_model.inputs, inp_type], outputs=out)
