...
#
#
# class PairDiscriminateModel(BaseModel):
#
#     def __init__(self,
...
#         :param kwargs:
#         """
#         super(PairDiscriminateModel, self).__init__(embedding, hyper_parameters, **kwargs)
...
#
#     def info(self):
#         info = super(PairDiscriminateModel, self).info()
...
#         agent: PairDiscriminateModel = super(PairDiscriminateModel, cls).load_model(model_path)
...
#                            x_data_a: Union[List[List[str]], List[List[List[str]]]],
#                            x_data_b: Union[List[List[str]], List[List[List[str]]]],
#                            y_data: List[str],
#                            batch_size: int = 64,
#                            is_bert: bool = False):
#         x_data_level = depth_count(x_data_a)
#         if x_data_level == 2:
#             x_data_a = [x_data_a]
#             x_data_b = [x_data_b]
#         data_len = len(y_data)
...
#                 target_x_b = []
#                 for x_a in x_data_a:
#                     target_x_a.append(x_a[start_index: end_index])
#                 for x_b in x_data_b:
#                     target_x_b.append(x_b[start_index: end_index])
#                 target_y = y_data[start_index: end_index]
#                 if len(target_x_a[0]) == 0:
#                     for x_a in x_data_a:
#                         target_x_a.append(x_a[0: batch_size])
#                     for x_b in x_data_b:
#                         target_x_b.append(x_b[0: batch_size])
#                     target_y = y_data[0: batch_size]
#
#                 padded_x_a = []
#                 padded_x_b = []
#                 for i, x_a in enumerate(target_x_a):
#                     x_b = target_x_b[i]
#                     tokenized_x_a = self.embedding.tokenize(x_a)
#                     tokenized_x_b = self.embedding.tokenize(x_b)
#
#                     if isinstance(self.embedding.sequence_length, int):
#                         padded_x_a.append(sequence.pad_sequences(tokenized_x_a,
#                                                                  maxlen=self.embedding.sequence_length,
#                                                                  padding='post')
#                                           )
#                         padded_x_b.append(sequence.pad_sequences(tokenized_x_b,
#                                                                  maxlen=self.embedding.sequence_length,
#                                                                  padding='post')
#                                           )
...
#                 if is_bert:
#                     if isinstance(self.embedding.sequence_length, int):
#                         padded_x_a_seg = [np.zeros(shape=(len(padded_x_i),
#                                                           self.embedding.sequence_length))
#                                           for padded_x_i in padded_x_a]
#                         padded_x_b_seg = [np.zeros(shape=(len(padded_x_i),
#                                                           self.embedding.sequence_length))
#                                           for padded_x_i in padded_x_b]
#                     elif isinstance(self.embedding.sequence_length, list):
#                         padded_x_a_seg = [np.zeros(shape=(len(padded_x_i),
#                                                           self.embedding.sequence_length[i]))
#                                           for i, padded_x_i in enumerate(padded_x_a)]
#                         padded_x_b_seg = [np.zeros(shape=(len(padded_x_i),
#                                                           self.embedding.sequence_length[i]))
#                                           for i, padded_x_i in enumerate(padded_x_b)]
#                     x_a_input_data = list(chain(*[(x, x_seg)
#                                                   for x, x_seg in zip(padded_x_a, padded_x_a_seg)]))
#                     x_b_input_data = list(chain(*[(x, x_seg)
#                                                   for x, x_seg in zip(padded_x_b, padded_x_b_seg)]))
#                 else:
#                     x_a_input_data = padded_x_a[0] if x_data_level == 2 else padded_x_a
#                     x_b_input_data = padded_x_b[0] if x_data_level == 2 else padded_x_b
#                 yield (x_a_input_data, x_b_input_data, padded_y)
#
#     def fit(self,
#             x_a_train: Union[List[List[str]], List[List[List[str]]]],
#             x_b_train: Union[List[List[str]], List[List[List[str]]]],
#             y_train: Union[List[str], List[List[str]], List[Tuple[str]]],
...
#         """
#
#         :param x_a_train: list of training data.
#         :param x_b_train: list of training data.
...
#         :return:
#         """
#         x_train_level = depth_count(x_a_train)
#         if x_train_level == 2:
#             assert len(x_a_train) == len(y_train)
#         elif x_train_level > 2:
#             for x_part in x_a_train:
