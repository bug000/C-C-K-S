...
def token_stream(line: str, ngram: int):
    re_list = []
    line_list = list(line)
    # print line
    # print line_list
...
def extract_entity_text(entity_json_line: dict):
    """
    ...
    for entity_str in all_str:
        tokens = token_stream(entity_str, ngram)
        all_tokens += tokens
    return list(filter(lambda token: token != "", all_tokens))


def compute_zh_cos_sim(text_a: list, text_b: list):
    text_a = set(text_a)
    text_b = set(text_b)
...
subject_dic = get_kb_dic()


def get_subject_id(mention_text: str, pre_text: str):
    """
    ...
    :return:
    """
    entity_json_line_s = subject_dic.get(mention_text, [])

    cos_temp = -1.0
    subject_id_temp = "NIL"

    for entity_json_line in entity_json_line_s:
        entity_message_text = extract_entity_text(entity_json_line)
        cos_sim = compute_zh_cos_sim(entity_message_text, token_stream(pre_text, 2))
...
# dev_path = r"Z:/research/data/biendata/ccks2019_el/ccks2019_el/develop.json"
dev_path = r"D:\data\biendata\ccks2019_el\ccks_train_data\validate.json"
# result_path = r"Z:/research/data/biendata/ccks2019_el/ccks2019_el/result.json"
result_path = r"D:\data\biendata\ccks2019_el\ccks_train_data\validate.pre.nil.json"

# model_path = r"D:\data\biendata\ccks2019_el\ccks_train_data\none_type_model"
...
with tf.device('/gpu:0'):
    ...
    pre_seq_s = model.predict(x_pre, batch_size=1024)
...
for data_ind, pre_seq in enumerate(tqdm(pre_seq_s)):
        ...
        if entity_mark.startswith(BEGIN) or entity_mark.startswith(SINGLE):
            ...
            for token_id_i in range(token_id, len(pre_seq)):
                if pre_seq[token_id_i] is NONE:
                    subject_id_m = get_subject_id(mention["mention"], "".join(dev_text))
    ...
    result_writer.write(json.dumps(dev_line, ensure_ascii=False) + "\n")
    result_writer.flush()
